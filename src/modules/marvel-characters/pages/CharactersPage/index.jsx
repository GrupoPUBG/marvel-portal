...
function Peticion() {
  if (select == 'A-Z') {
    ...
    if (modalF == false) {
      if (filterEnter == true) {
        if (updated.length < 1) {
          return <CharactersGrid n={3} />;
        } else {
          return <CharactersGrid n={4} search={search} />;
        }
      } else {
        return <CharactersGrid n={3} search={search} />;
      }
    } else {
      return <CharactersGrid n={5} search={letter} />;
    }
  }
}
function Peticion2() {
  if (search.length >= 3) {
    return <ModalAuto search={search} modalAuto={modalAuto} closeModalA={closeModalA}/>;
  }
}
return (
